    ...
    <div className="bg-[#1F1F1F] rounded-xl border border-[#2D2D2D] overflow-hidden">
      <div className="flex items-center justify-between px-5 pt-4 pb-3">
        <div className="flex items-center gap-3">
          ...
          {totalMembers > 0 && (
            <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-[rgba(99,102,241,0.1)] text-indigo-400">
        ...
        <div className="px-5 pb-4 space-y-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex items-center gap-3 animate-pulse">
              <div className="w-8 h-8 bg-[#2D2D2D] rounded-full shrink-0" />
              <div className="flex-1">
                <div className="h-3 bg-[#2D2D2D] rounded w-32 mb-1.5" />
                <div className="h-2 bg-[#161B22] rounded w-44" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="px-5 pb-5">
          <div className="flex flex-col items-center justify-center py-6 text-center">
            <div className="p-3 rounded-full bg-[#2D2D2D] mb-3">
              <Users size={20} weight="duotone" className="text-gray-500" />
            </div>
            <p className="text-xs text-gray-500">{t('dashboard.home.no_members_yet')}</p>
          </div>
        </div>
      ) : (
        <div className="divide-y divide-[#2D2D2D]">
          {members.map((member: any) => {
            ...
            return (
              ...
              >
                {/* Avatar */}
                <div className="w-8 h-8 rounded-full bg-[rgba(99,102,241,0.15)] shrink-0 flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!user.email_verified && (
                      <span className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-[rgba(245,158,11,0.1)] text-amber-400">
                ...
                {/* Role badge */}
                {role && (
                  <span className="flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-[#2D2D2D] text-gray-400 shrink-0">
                    <ShieldCheck size={10} />
                    {role.name}
                  </span>
                )}
