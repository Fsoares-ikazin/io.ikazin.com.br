      ...
      <div className={`${isFloating ? 'flex justify-between' : 'grid grid-cols-3'} items-center w-full`}>
        {isFloating ? (
          ...
          <>
            ...
            >
              <ChevronLeft size={20} className="text-gray-100 shrink-0" />
            ...
            >
              ...
              <ChevronRight size={20} className="text-gray-100 shrink-0" />
            </button>
          </>
        ) : (
            ...
            <div className="justify-self-start">
              <button
                ...
                className={`flex items-center space-x-1.5 px-3.5 py-2 rounded-md transition-all duration-200 cursor-pointer ${
                  prevActivity
                    ? 'bg-ikz-surface shadow-lg shadow-black/30 text-gray-300'
                    : 'bg-ikz-surface text-gray-400 cursor-not-allowed'
                }`}
                disabled={!prevActivity}
                title={prevActivity ? `${t('common.previous')}: ${prevActivity.name}` : t('activities.no_previous_activity')}
              >
    ...
    <>
      ...
      {!isBottomNavVisible && (
        <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-sticky-header w-[85%] sm:w-auto sm:min-w-[350px] max-w-lg transition-all duration-300 ease-in-out">
          <div
            className="bg-ikz-surface/90 backdrop-blur-xl rounded-full py-1.5 px-2.5 shadow-xs animate-in fade-in slide-in-from-bottom duration-300"
          >
